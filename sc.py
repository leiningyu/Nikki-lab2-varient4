...
class ImmutableNode:
    ...
    def __str__(self):
        return f"{self._value} -> {self._next}" if self._next else str(self._value)
...
class ImmutableHashTable:
    def __init__(self, buckets=None, size=10):
        self._size = size
        self._buckets = buckets if buckets is not None else tuple(None for _ in range(size))
    ...
    def _with_updated_bucket(self, index: int, new_head) -> 'ImmutableHashTable':
...
def cons(value, ht: ImmutableHashTable) -> ImmutableHashTable:
    ...
    # Check repetition
    def contains(node):
        return node is not None and (node._value == value or contains(node._next))
 
    if contains(current):
        return ht
 
    # Insert new node
    return ht._with_updated_bucket(index, ImmutableNode(value, current))
...
def is_member(ht: ImmutableHashTable, value) -> bool:
    index = ht._hash(value)
    def check(node):
        return node is not None and (node._value == value or check(node._next))
    return check(ht._get_bucket(index))

def reverse(ht: ImmutableHashTable) -> ImmutableHashTable:
    def reverse_bucket(node, acc=None):
        return acc if node is None else reverse_bucket(node._next, ImmutableNode(node._value, acc))
...
def intersection(a: ImmutableHashTable, b: ImmutableHashTable) -> ImmutableHashTable:
...
def to_list(ht: ImmutableHashTable) -> list:
    ...
    def collect(node, acc):
        return acc if node is None else collect(node._next, [node._value] + acc)
...
def filter(ht: ImmutableHashTable, predicate: Callable[[Any], bool]) -> ImmutableHashTable:
...
def map(ht: ImmutableHashTable, func: Callable[[Any], Any]) -> ImmutableHashTable:
    def map_bucket(node):
        return None if node is None else ImmutableNode(func(node._value), map_bucket(node._next))
    
    return ImmutableHashTable(tuple(map_bucket(bucket) for bucket in ht._buckets), ht._size)

def ht_reduce(ht: ImmutableHashTable, func: Callable[[Any, Any], Any], initial) -> Any:
    acc = initial
    for bucket in ht._buckets:
        current = bucket
        while current:
...
def equals(a: ImmutableHashTable, b: ImmutableHashTable) -> bool:
    def compare_buckets(bucket_a, bucket_b):
        ...
        if bucket_a is None or bucket_b is None:
            return False
        return bucket_a._value == bucket_b._value and compare_buckets(bucket_a._next, bucket_b._next)
    
    return all(compare_buckets(ba, bb) for ba, bb in zip(a._buckets, b._buckets))
